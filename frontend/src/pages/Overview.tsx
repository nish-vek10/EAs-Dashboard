...
import { fetchAccountsFiltered, type AccountRow } from "../api";
...
export default function Overview({ filter }: { filter: Group }) {
  ...
  // Live updates merged into current list
  useEffect(() => {
    const unsubscribe = subscribe((evt: LiveEvent) => {
      ...
      setAccounts((prev) => {
        ...
        const row =
          map.get(evt.account) ??
          ({ login_hint: evt.account, positions_count: 0 } as AccountRow);
  ...
  const visible = useMemo(() => {
    const arr = [...accounts];
    switch (sortKey) {
      case "equity_asc":  arr.sort((a, b) => cmpNumberAsc(getEquity(a), getEquity(b))); break;
      case "equity_desc": arr.sort((a, b) => cmpNumberAsc(getEquity(b), getEquity(a))); break;
      case "net_asc":     arr.sort((a, b) => cmpNumberAsc(getNetPct(a), getNetPct(b))); break;
      case "net_desc":    arr.sort((a, b) => cmpNumberAsc(getNetPct(b), getNetPct(a))); break;
      case "alpha":
      default:            arr.sort((a, b) => labelKey(a).localeCompare(labelKey(b))); break;
    }
    return arr;
  }, [accounts, sortKey]);
